...
export const PublishingManager: React.FC<PublishingManagerProps> = ({
  ...
  const statuses: { id: PageStatus; label: string; color: string; icon: any }[] = [
    ...
    { id: 'Archived', label: 'Archived', color: 'bg-slate-800 text-slate-400 border-slate-700', icon: Archive }
  ];

  return (
    <div className="flex-1 bg-slate-950 p-6 text-slate-100 overflow-y-auto">
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <div>
          ...
          <h1 className="text-2xl font-black text-slate-100">Publishing & Edge A/B Routing Settings</h1>
          <p className="text-xs text-slate-400 mt-1">Configure domain resolution, custom SSL certificates, lifecycle states, and edge traffic split testing.</p>
        </div>

        {/* SECTION 1: LIFECYCLE STATE MACHINE */}
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-4">
          <h3 className="text-sm font-bold text-slate-200">Funnel Step Lifecycle State Machine</h3>
            ...
            {statuses.map((st) => {
              ...
              return (
                <button 
                  ...
                  className={`p-4 rounded-xl border text-left transition-all ${isCurrent ? `${st.color} shadow-lg ring-2 ring-indigo-500` : 'bg-slate-950 border-slate-800 text-slate-400 hover:border-slate-700'}`}
                >
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-6">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-bold text-slate-200">Domain Mapping & Edge SSL Resolution</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-semibold text-slate-300 mb-1">Multi-Tenant Subdomain</label>
              <div className="flex items-center bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-xs text-slate-300">
                <input 
                  type="text" 
                  value={subdomain}
                  onChange={(e) => setSubdomain(e.target.value)}
                  className="bg-transparent font-mono text-white focus:outline-none w-full"
                />
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-300 mb-1">Custom Domain Alias</label>
              <input 
                type="text" 
                value={customDomain}
          ...
          <div className="p-4 bg-slate-950 border border-slate-800 rounded-xl flex items-center justify-between">
            <div>
              <div className="text-[10px] font-extrabold text-slate-400 uppercase tracking-wider">RESOLVED LIVE STEP URL</div>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-bold text-slate-200">Edge-Based A/B Split Test Engine</h3>
            </div>
            <button 
              onClick={() => onUpdateStepAbTest(activeStep.id, !activeStep.abSplitEnabled, trafficSplit)}
              className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${activeStep.abSplitEnabled ? 'bg-indigo-600 text-white' : 'bg-slate-800 text-slate-400 hover:text-white'}`}
            >
          ...
          {activeStep.abSplitEnabled && (
            <div className="space-y-4 pt-2 border-t border-slate-800">
              <div className="flex items-center justify-between text-xs">
                <span className="font-semibold text-slate-300">Traffic Split Distribution:</span>
              ...
              <div className="grid grid-cols-2 gap-4 pt-2">
                <div className="p-4 bg-slate-950 rounded-xl border border-indigo-500/40 text-center">
                  <div className="text-xs font-bold text-indigo-400">Variant A (Control)</div>
                  <div className="text-xl font-black text-white mt-1">4.82% Conv</div>
                  <div className="text-[10px] text-slate-400">1,420 Visitors</div>
                </div>
                <div className="p-4 bg-slate-950 rounded-xl border border-pink-500/40 text-center">
                  <div className="text-xs font-bold text-pink-400">Variant B (Challenger)</div>
                  <div className="text-xl font-black text-white mt-1">6.14% Conv</div>
                  <div className="text-[10px] text-slate-400">1,390 Visitors</div>
                </div>
              </div>
            </div>
          )}
