      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-4xl max-h-[90vh] flex flex-col shadow-2xl text-slate-100 overflow-hidden">
        ...
        <div className="p-6 border-b border-slate-800 flex items-center justify-between">
          <div>
            ...
            <h2 className="text-xl font-black text-slate-100">Funnel Archetype Template Library</h2>
          </div>
        ...
        <div className="p-4 border-b border-slate-800 flex items-center gap-2 bg-slate-950/50">
          ...
          {categories.map((cat) => (
            <button 
              ...
              className={`px-4 py-1.5 rounded-lg text-xs font-extrabold transition-all ${selectedCategory === cat ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-600/30' : 'bg-slate-800 text-slate-400 hover:text-slate-200'}`}
            >
              {cat} Funnels
            </button>
          ))}
        ...
        <div className="flex-1 overflow-y-auto p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
          {/* Empty Canvas Option */}
          <div className="bg-slate-900 border-2 border-dashed border-indigo-500/50 hover:border-indigo-400 rounded-2xl p-5 flex flex-col justify-center items-center group transition-all cursor-pointer shadow-lg hover:shadow-indigo-500/20"
            onClick={() => {
              onInstantiateTemplate({
                id: `empty_${Date.now()}`,
                name: 'Empty Funnel',
          ...
          >
            <div className="w-16 h-16 rounded-full bg-slate-800/80 text-slate-400 group-hover:text-indigo-400 flex items-center justify-center mb-4 transition-colors">
              <Layers className="w-8 h-8" />
            </div>
            <h3 className="text-xl font-bold text-slate-100 group-hover:text-indigo-400 transition-colors mb-2">Empty Canvas</h3>
          ...
          {filteredTemplates.map((tpl) => {
            ...
            return (
            <div key={tpl.id} className="bg-slate-950 border border-slate-800 hover:border-indigo-500/60 rounded-2xl p-5 flex flex-col justify-between group transition-all">
              <div>
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {isColorVariant && (
                      <span
                        className="w-4 h-4 rounded-full border-2 border-slate-700 flex-shrink-0"
                ...
                <h3 className="text-lg font-bold text-slate-100 group-hover:text-indigo-400 transition-colors mb-2">{tpl.name}</h3>
                ...
                <div className="space-y-1.5 mb-6">
                  {tpl.steps.map((s) => (
                    <div key={s.id} className="text-[11px] text-slate-300 flex items-center gap-2 bg-slate-900/80 px-3 py-1.5 rounded-lg border border-slate-800">
                      <span className="w-4 h-4 rounded-full bg-slate-800 text-slate-400 font-bold text-[9px] flex items-center justify-center">{s.stepOrder}</span>
              ...
              <div className="flex items-center gap-2 pt-4 border-t border-slate-800/80">
                <button 
                  ...
                  className="flex-1 py-2.5 text-white font-extrabold rounded-xl text-xs shadow-lg flex items-center justify-center gap-2 transition-all"
                ...
                <button 
                  ...
                  className="p-2.5 bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white rounded-xl text-xs font-bold"
        ...
        {shareToken && (
          <div className="p-4 bg-indigo-950/60 border-t border-indigo-500/40 flex items-center justify-between text-xs text-indigo-200">
            <div className="flex items-center gap-2">
              ...
              <span>Share Token: <strong className="font-mono text-white">{shareToken}</strong></span>
            </div>
