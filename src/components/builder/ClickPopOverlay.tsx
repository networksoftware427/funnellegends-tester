    ...
    <div className="fixed inset-0 z-[100] bg-slate-950/85 backdrop-blur-md flex items-center justify-center p-4 animate-fade-in overflow-y-auto">
      ...
      <div className="relative w-full max-w-2xl my-auto">
          ...
          <div className="bg-white border border-indigo-200 rounded-3xl overflow-hidden shadow-2xl text-slate-900 grid grid-cols-1 md:grid-cols-12 relative">
            ...
            <div className="md:col-span-5 bg-[url('https://images.unsplash.com/photo-1557682224-5b8590cd9ec5?w=800&auto=format&fit=crop&q=80')] bg-cover bg-center p-6 flex flex-col items-center justify-center border-b md:border-b-0 md:border-r border-slate-200 text-center relative overflow-hidden">
              ...
              <div className="w-32 h-44 bg-gradient-to-br from-indigo-500 to-blue-600 rounded-lg border-2 border-indigo-300 shadow-[0_20px_40px_rgba(0,0,0,0.5)] p-4 flex flex-col justify-between transform -rotate-3 hover:rotate-0 transition-transform relative z-10">
                ...
                <div className="text-[11px] font-black text-white text-left leading-tight drop-shadow-md">2026 FUNNEL ARCHITECTURE BLUEPRINT</div>
        ...
        {layoutVariant === 'cyber_exit' && (
          <div className="bg-slate-950 border-2 border-pink-500 rounded-3xl overflow-hidden shadow-[0_0_50px_rgba(236,72,153,0.25)] text-slate-100 p-6 md:p-8 space-y-5 text-center relative bg-[url('https://images.unsplash.com/photo-1555680202-c86f0e12f086?w=800&auto=format&fit=crop&q=80')] bg-cover bg-center bg-blend-overlay">
            <div className="absolute inset-0 bg-slate-950/85 backdrop-blur-sm z-0"></div>
            <div className="relative z-10 space-y-5">
              <button onClick={onClose} className="absolute -top-2 -right-2 z-20 p-2 bg-slate-900 text-pink-400 hover:text-pink-300 hover:bg-slate-800 rounded-full border border-pink-500/40 transition-colors">
                <X className="w-5 h-5" />
              </button>
              ...
              <div className="space-y-2">
                <h2 className="text-2xl md:text-3xl font-black text-white tracking-tight drop-shadow-md">{settings.title}</h2>
                <p className="text-xs md:text-sm text-pink-100/90 max-w-md mx-auto">{settings.subtitle}</p>
              </div>
              <div className="p-3 bg-slate-900/90 border border-pink-500/50 rounded-2xl max-w-md mx-auto flex items-center justify-between font-mono text-xs shadow-lg shadow-pink-500/10">
              ...
              {!isSubmitted ? (
                <form onSubmit={handleOptinSubmit} className="space-y-3 max-w-md mx-auto">
                  <input type="email" required placeholder="Enter email to lock $150 promo voucher..." value={emailInput} onChange={(e) => setEmailInput(e.target.value)} className="w-full bg-slate-900/90 border border-pink-900 focus:border-pink-500 focus:ring-1 focus:ring-pink-500 outline-none rounded-xl px-4 py-3 text-xs text-white placeholder-slate-500 transition-all" />
                ...
                <div className="p-4 bg-slate-900/90 rounded-xl border border-emerald-500/50 text-center animate-fade-in">
        ...
        {layoutVariant === 'webinar_pass' && (
          <div className="bg-emerald-950 border border-emerald-500/40 rounded-3xl overflow-hidden shadow-[0_0_40px_rgba(16,185,129,0.15)] text-slate-100 p-6 md:p-8 space-y-5 text-left relative bg-[url('https://images.unsplash.com/photo-1598488035139-bdbb2231ce04?w=800&auto=format&fit=crop&q=80')] bg-cover bg-center bg-blend-overlay">
            <div className="absolute inset-0 bg-slate-950/85 z-0 backdrop-blur-[2px]"></div>
            <div className="relative z-10 space-y-5">
              <button onClick={onClose} className="absolute -top-2 -right-2 z-20 p-2 bg-slate-900 text-slate-400 hover:text-white rounded-full border border-slate-700 transition-colors">
                <X className="w-4 h-4" />
              </button>
              ...
              <div className="space-y-2">
                <h2 className="text-xl md:text-2xl font-black text-white leading-tight">{settings.title}</h2>
                <p className="text-xs text-slate-300 leading-relaxed font-medium">{settings.subtitle}</p>
              </div>
              {/* Host Avatar Row */}
              <div className="flex items-center gap-3 p-3 bg-slate-900/80 rounded-xl border border-emerald-900/50 shadow-inner">
                ...
                <div className="text-xs">
                  <div className="font-bold text-slate-100">Host: Mark Vance (Funnel Architect)</div>
              ...
              {!isSubmitted ? (
                <form onSubmit={handleOptinSubmit} className="space-y-3">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-2.5">
                    <input type="text" required placeholder="First Name..." value={nameInput} onChange={(e) => setNameInput(e.target.value)} className="bg-slate-900/90 border border-slate-700 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 outline-none rounded-xl px-3.5 py-2.5 text-xs text-white transition-all" />
                    <input type="email" required placeholder="Best Email Address..." value={emailInput} onChange={(e) => setEmailInput(e.target.value)} className="bg-slate-900/90 border border-slate-700 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 outline-none rounded-xl px-3.5 py-2.5 text-xs text-white transition-all" />
                  </div>
                ...
                <div className="p-4 bg-slate-900/90 rounded-xl border border-emerald-500/40 text-center animate-fade-in">
                  <CheckCircle className="w-8 h-8 text-emerald-400 mx-auto mb-2" />
        ...
        {layoutVariant === 'gold_luxury' && (
          <div className="bg-slate-950 border-2 border-amber-500/50 rounded-3xl overflow-hidden shadow-[0_0_40px_rgba(245,158,11,0.15)] text-slate-100 p-6 md:p-8 space-y-5 text-left relative bg-[url('https://images.unsplash.com/photo-1606132338520-2228fbb44005?w=800&auto=format&fit=crop&q=80')] bg-cover bg-center bg-blend-multiply">
            <div className="absolute inset-0 bg-slate-950/80 z-0"></div>
            <div className="relative z-10 space-y-5">
              <button onClick={onClose} className="absolute -top-2 -right-2 z-20 p-2 bg-slate-900 text-amber-500 hover:text-amber-400 hover:bg-slate-800 rounded-full border border-amber-500/30 transition-colors">
                <X className="w-4 h-4" />
              </button>
              ...
              <div className="space-y-2">
                <h2 className="text-xl md:text-2xl font-black text-white leading-tight drop-shadow-md">{settings.title}</h2>
                <p className="text-xs text-slate-300 leading-relaxed font-medium">{settings.subtitle}</p>
              </div>
              {!isSubmitted ? (
                <form onSubmit={handleOptinSubmit} className="space-y-3">
                  <input type="text" required placeholder="Full Name..." value={nameInput} onChange={(e) => setNameInput(e.target.value)} className="w-full bg-slate-900/80 border border-slate-700 focus:border-amber-500 focus:ring-1 focus:ring-amber-500 outline-none rounded-xl px-3.5 py-2.5 text-xs text-white transition-all" />
                  <input type="email" required placeholder="Work Email..." value={emailInput} onChange={(e) => setEmailInput(e.target.value)} className="w-full bg-slate-900/80 border border-slate-700 focus:border-amber-500 focus:ring-1 focus:ring-amber-500 outline-none rounded-xl px-3.5 py-2.5 text-xs text-white transition-all" />
                  <input type="tel" placeholder="Mobile Phone for SMS Confirmation..." value={phoneInput} onChange={(e) => setPhoneInput(e.target.value)} className="w-full bg-slate-900/80 border border-slate-700 focus:border-amber-500 focus:ring-1 focus:ring-amber-500 outline-none rounded-xl px-3.5 py-2.5 text-xs text-white transition-all" />
        ...
        {layoutVariant === 'saas_card' && (
          <div className="bg-gradient-to-br from-violet-900 via-slate-950 to-fuchsia-950 border border-violet-500/40 rounded-3xl overflow-hidden shadow-2xl text-slate-100 p-6 md:p-8 space-y-5 text-center relative bg-[url('https://images.unsplash.com/photo-1550745165-9bc0b252726f?w=800&auto=format&fit=crop&q=80')] bg-cover bg-center bg-blend-overlay">
            <div className="absolute inset-0 bg-slate-950/80 backdrop-blur-sm z-0"></div>
            <div className="relative z-10 space-y-5">
              <button onClick={onClose} className="absolute -top-2 -right-2 z-20 p-2 bg-slate-900 text-slate-400 hover:text-white rounded-full border border-slate-700 transition-colors">
                <X className="w-4 h-4" />
              </button>
              <span className="inline-block bg-violet-600 text-white font-extrabold text-xs px-4 py-1.5 rounded-full shadow-[0_0_15px_rgba(139,92,246,0.4)] uppercase tracking-wider">
                {settings.badgeText}
              </span>
              <div className="space-y-2 max-w-lg mx-auto">
                <h2 className="text-2xl font-black text-white drop-shadow-md">{settings.title}</h2>
              ...
              <div className="flex flex-wrap items-center justify-center gap-2 text-[10px] font-semibold text-violet-200">
                <span className="bg-slate-900/80 px-3 py-1.5 rounded-lg border border-violet-500/30 backdrop-blur-sm">✓ Visual Builder Engine</span>
                <span className="bg-slate-900/80 px-3 py-1.5 rounded-lg border border-violet-500/30 backdrop-blur-sm">✓ Stripe Token Vault</span>
                <span className="bg-slate-900/80 px-3 py-1.5 rounded-lg border border-violet-500/30 backdrop-blur-sm">✓ AI VSL Copywriter</span>
              </div>
              {!isSubmitted ? (
                <form onSubmit={handleOptinSubmit} className="space-y-3 max-w-md mx-auto pt-2">
                  <input type="email" required placeholder="Enter work email address..." value={emailInput} onChange={(e) => setEmailInput(e.target.value)} className="w-full bg-slate-900/90 border border-violet-900 focus:border-violet-500 focus:ring-1 focus:ring-violet-500 outline-none rounded-xl px-4 py-3 text-xs text-white placeholder-slate-400 transition-all" />
                  <button type="submit" className="w-full bg-violet-600 hover:bg-violet-500 text-white font-extrabold py-3.5 rounded-xl text-xs shadow-lg shadow-violet-600/40 flex items-center justify-center gap-2 transition-transform hover:-translate-y-0.5">
                    <span>{settings.buttonText}</span>
                    <ArrowRight className="w-4 h-4" />
                  </button>
                </form>
        ...
        {layoutVariant === 'editorial' && (
          <div className="bg-[#fdfbf7] border border-[#e5e0d8] rounded-3xl overflow-hidden shadow-2xl text-slate-900 p-6 md:p-8 space-y-5 text-left font-serif relative">
            <button onClick={onClose} className="absolute top-4 right-4 z-20 p-2 bg-white text-slate-400 hover:text-slate-700 hover:bg-slate-50 rounded-full border border-slate-200 font-sans shadow-sm transition-colors">
              <X className="w-4 h-4" />
            </button>
        ...
        {layoutVariant === 'flash_scarcity' && (
          <div className="bg-gradient-to-b from-rose-600 to-red-950 border-4 border-red-500 rounded-3xl overflow-hidden shadow-[0_0_50px_rgba(225,29,72,0.3)] text-white p-6 md:p-8 space-y-5 text-center relative bg-[url('https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?w=800&auto=format&fit=crop&q=80')] bg-cover bg-center bg-blend-overlay">
            <div className="absolute inset-0 bg-red-950/70 z-0"></div>
            <div className="relative z-10 space-y-5">
              <button onClick={onClose} className="absolute -top-2 -right-2 z-20 p-2 bg-slate-900 text-slate-300 hover:text-white rounded-full border border-red-500/50 transition-colors">
                <X className="w-4 h-4" />
              </button>
              ...
              <div className="space-y-2">
                <h2 className="text-2xl font-black text-white drop-shadow-lg">{settings.title}</h2>
                <p className="text-xs text-rose-200/90 max-w-md mx-auto font-medium">{settings.subtitle}</p>
              </div>
              {/* Live Ticking Scarcity Digital Clock */}
              <div className="flex items-center justify-center gap-3 bg-slate-950/90 p-3.5 rounded-2xl border border-red-500/50 max-w-xs mx-auto font-mono shadow-inner">
                <div className="text-center"><span className="text-2xl font-black text-red-500">00</span><div className="text-[9px] text-slate-400 font-sans tracking-widest">HRS</div></div>
                <span className="text-red-600 font-bold text-xl pb-3">:</span>
                <div className="text-center"><span className="text-2xl font-black text-red-500">14</span><div className="text-[9px] text-slate-400 font-sans tracking-widest">MIN</div></div>
                <span className="text-red-600 font-bold text-xl pb-3">:</span>
                <div className="text-center"><span className="text-2xl font-black text-red-500 animate-pulse">59</span><div className="text-[9px] text-slate-400 font-sans tracking-widest">SEC</div></div>
              </div>
              {!isSubmitted ? (
                <form onSubmit={handleOptinSubmit} className="space-y-3 max-w-md mx-auto pt-1">
                  <input type="email" required placeholder="Enter email to claim $97 bundle..." value={emailInput} onChange={(e) => setEmailInput(e.target.value)} className="w-full bg-slate-900/90 border border-red-900 focus:border-red-500 focus:ring-1 focus:ring-red-500 outline-none rounded-xl px-4 py-3 text-xs text-white placeholder-slate-400 transition-all" />
        ...
        {layoutVariant === 'vsl_cinema' && (
          <div className="bg-slate-950 border border-indigo-500/40 rounded-3xl overflow-hidden shadow-2xl text-slate-100 p-6 md:p-8 space-y-5 text-center relative bg-[url('https://images.unsplash.com/photo-1595769816263-9b910be24d5f?w=800&auto=format&fit=crop&q=80')] bg-cover bg-center bg-blend-multiply">
            ...
            <div className="relative z-10 space-y-5">
              ...
              <div className="space-y-2">
                <h2 className="text-xl md:text-2xl font-black text-white drop-shadow-md">{settings.title}</h2>
                <p className="text-xs text-slate-300 font-medium">{settings.subtitle}</p>
              </div>
              {/* Video Thumbnail Frame */}
              <div className="relative aspect-video max-w-md mx-auto rounded-2xl overflow-hidden border-2 border-indigo-500/30 bg-slate-900 shadow-xl shadow-indigo-900/20 flex items-center justify-center group">
              ...
              {!isSubmitted ? (
                <form onSubmit={handleOptinSubmit} className="space-y-3 max-w-md mx-auto">
                  <input type="email" required placeholder="Enter email to unlock video player..." value={emailInput} onChange={(e) => setEmailInput(e.target.value)} className="w-full bg-slate-900/90 border border-slate-700 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 outline-none rounded-xl px-4 py-3 text-xs text-white placeholder-slate-500 transition-all" />
        ...
        {layoutVariant === 'beta_glass' && (
          <div className="bg-slate-900/70 backdrop-blur-2xl border border-purple-500/40 rounded-3xl overflow-hidden shadow-[0_0_40px_rgba(168,85,247,0.2)] text-slate-100 p-6 md:p-8 space-y-5 text-left relative bg-[url('https://images.unsplash.com/photo-1550751827-4bd374c3f58b?w=800&auto=format&fit=crop&q=80')] bg-cover bg-center bg-blend-overlay">
            <div className="absolute inset-0 bg-slate-950/80 backdrop-blur-md z-0"></div>
            <div className="relative z-10 space-y-5">
              <button onClick={onClose} className="absolute -top-2 -right-2 z-20 p-2 bg-slate-900/80 text-slate-400 hover:text-white rounded-full border border-slate-700 transition-colors">
                <X className="w-4 h-4" />
              </button>
              ...
              {!isSubmitted ? (
                <form onSubmit={handleOptinSubmit} className="space-y-3">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-2.5">
                    <input type="email" required placeholder="Work Email..." value={emailInput} onChange={(e) => setEmailInput(e.target.value)} className="bg-slate-950/80 border border-slate-700 focus:border-purple-500 focus:ring-1 focus:ring-purple-500 outline-none rounded-xl px-3.5 py-2.5 text-xs text-white transition-all" />
                    <input type="text" placeholder="Company Website URL..." value={companyInput} onChange={(e) => setCompanyInput(e.target.value)} className="bg-slate-950/80 border border-slate-700 focus:border-purple-500 focus:ring-1 focus:ring-purple-500 outline-none rounded-xl px-3.5 py-2.5 text-xs text-white transition-all" />
                  </div>
                ...
                <div className="p-4 bg-slate-950/80 rounded-xl border border-emerald-500/40 text-center animate-fade-in">
                  <CheckCircle className="w-8 h-8 text-emerald-400 mx-auto mb-2" />
        ...
        {layoutVariant === 'spin_wheel' && (
          <div className="bg-slate-950 border-4 border-amber-400 rounded-3xl overflow-hidden shadow-[0_0_50px_rgba(251,191,36,0.3)] text-slate-100 p-6 md:p-8 space-y-5 text-center relative bg-[url('https://images.unsplash.com/photo-1513151233558-d860c5398176?w=800&auto=format&fit=crop&q=80')] bg-cover bg-center bg-blend-overlay">
            <div className="absolute inset-0 bg-slate-950/85 z-0"></div>
            <div className="relative z-10 space-y-5">
              <button onClick={onClose} className="absolute -top-2 -right-2 z-20 p-2 bg-slate-900 text-amber-400 hover:text-amber-300 hover:bg-slate-800 rounded-full border border-amber-500/40 transition-colors">
                <X className="w-4 h-4" />
              </button>
              ...
              <div className="w-28 h-28 rounded-full bg-gradient-to-r from-amber-400 via-pink-500 to-purple-600 p-1.5 mx-auto shadow-[0_0_30px_rgba(251,191,36,0.4)] animate-spin-slow">
                <div className="w-full h-full bg-slate-900 rounded-full flex flex-col items-center justify-center text-center p-2 border border-slate-700">
                  <span className="text-[10px] font-bold text-amber-400 tracking-wider">YOU WON</span>
                  <span className="text-sm font-black text-white drop-shadow-sm">$200 OFF</span>
                </div>
              </div>
              {!isSubmitted ? (
                <form onSubmit={handleOptinSubmit} className="space-y-3 max-w-md mx-auto pt-2">
                  <input type="email" required placeholder="Enter email to claim $200 mystery voucher..." value={emailInput} onChange={(e) => setEmailInput(e.target.value)} className="w-full bg-slate-900/90 border border-amber-900/50 focus:border-amber-500 focus:ring-1 focus:ring-amber-500 outline-none rounded-xl px-4 py-3 text-xs text-white placeholder-slate-400 transition-all" />
                  <button type="submit" className="w-full bg-gradient-to-r from-amber-500 via-pink-500 to-purple-600 hover:from-amber-400 hover:to-purple-500 text-white font-black py-4 rounded-xl text-xs shadow-lg shadow-amber-500/30 flex items-center justify-center gap-2 transition-transform hover:-translate-y-0.5 border border-white/20">
        ...
        {layoutVariant === 'default' && (
          <div className="relative max-w-xl w-full bg-gradient-to-b from-slate-900 to-slate-950 border border-pink-500/40 rounded-3xl overflow-hidden shadow-2xl space-y-0 text-slate-100">
            <button onClick={onClose} className="absolute top-4 right-4 z-20 p-2 bg-slate-950/80 hover:bg-slate-800 text-slate-300 rounded-full border border-slate-700 transition-colors">
              <X className="w-5 h-5" />
            </button>
            ...
            <div className="p-6 md:p-8 space-y-5 text-center">
              {settings.imageUrl && (
                <div className="max-w-xs mx-auto aspect-video rounded-2xl overflow-hidden border border-slate-800 shadow-xl">
                  <img src={settings.imageUrl} alt="Popup Offer" className="w-full h-full object-cover" />
                </div>
              )}
              <div className="space-y-2">
                <h2 className="text-2xl md:text-3xl font-black text-white leading-tight drop-shadow-md">{settings.title}</h2>
                <p className="text-xs md:text-sm text-slate-300 max-w-md mx-auto leading-relaxed font-medium">{settings.subtitle}</p>
              </div>
              {!isSubmitted ? (
                <form onSubmit={handleOptinSubmit} className="space-y-3 max-w-md mx-auto pt-2">
                  <div className="relative">
                    <Mail className="w-4 h-4 text-slate-400 absolute left-4 top-3.5" />
                    <input type="email" required placeholder="Enter your email address..." value={emailInput} onChange={(e) => setEmailInput(e.target.value)} className="w-full bg-slate-950 border border-slate-800 rounded-2xl pl-11 pr-4 py-3.5 text-xs text-slate-100 placeholder-slate-500 focus:outline-none focus:ring-1 focus:ring-pink-500 transition-all" />
                  </div>
                ...
                <div className="p-6 bg-slate-950 rounded-2xl border border-emerald-500/40 text-center space-y-3 animate-fade-in">
                  <CheckCircle className="w-12 h-12 text-emerald-400 mx-auto" />
                  <h3 className="text-lg font-black text-white">Success! Your Special Offer Has Been Unlocked.</h3>
                </div>
              )}
            </div>
          </div>
