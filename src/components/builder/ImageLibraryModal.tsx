...
export const ImageLibraryModal: React.FC<ImageLibraryModalProps> = ({ onClose, onSelect }) => {
  ...
  return (
    <div className="fixed inset-0 bg-slate-950/80 z-[200] flex items-center justify-center p-4">
      <div className="bg-slate-900 border border-slate-700 rounded-3xl w-full max-w-5xl shadow-2xl overflow-hidden flex flex-col h-[85vh]">
        {/* Header */}
        <div className="p-6 border-b border-slate-800 flex items-center justify-between shrink-0">
          <div>
            <h2 className="text-xl font-bold text-white">Image Library</h2>
            <p className="text-sm text-slate-400 mt-1">Select an SVG icon to use as an image</p>
          </div>
          <button 
            onClick={onClose}
        ...
        {/* Filters and Search */}
        <div className="p-4 border-b border-slate-800 flex flex-col sm:flex-row gap-4 shrink-0 bg-slate-900">
          <div className="relative flex-1">
            ...
            <input 
              ...
              className="w-full bg-slate-950 border border-slate-800 rounded-xl pl-10 pr-4 py-3 text-sm text-white focus:border-indigo-500 transition-colors"
            />
          </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
              {filteredImages.map(img => (
                <div 
                  ...
                  className="bg-slate-900 border border-slate-800 hover:border-indigo-500 rounded-2xl p-4 flex flex-col items-center gap-3 cursor-pointer group transition-all hover:shadow-lg hover:shadow-indigo-500/10"
                >
                  ...
                  <div className="text-center w-full">
                    <h3 className="text-xs font-bold text-slate-300 truncate w-full" title={img.name}>{img.name}</h3>
