...
export const AutomationWorkflowBuilder: React.FC = () => {
  ...
  const [activeSubTab, setActiveSubTab] = useState<'canvas' | 'simulations' | 'message_templates' | 'database'>('canvas');
  ...
  const handleRunSimulation = () => {
    ...
    const steps = [
      eventTitle,
      `🔍 [CONDITION] Verifying deduplication & suppression lists in Supabase CRM... (PASSED)`,
    ...
    <div className="flex-1 bg-slate-50 text-slate-900 overflow-y-auto flex flex-col font-sans">
      ...
      <div className="bg-white border-b border-slate-200 px-6 py-2 flex items-center gap-1.5 overflow-x-auto no-scrollbar shrink-0 shadow-sm">
        ...
        <button 
          ...
          className={`px-3.5 py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-2 shrink-0 ${activeSubTab === 'database' ? 'bg-emerald-600 text-white shadow-md shadow-emerald-600/25 font-black' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-100'}`}
        >
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              <div className="bg-white p-4 rounded-2xl border border-slate-200 space-y-1 shadow-sm">
                <span className="text-[10px] uppercase font-mono font-bold text-slate-500">Active Workflows</span>
                <div className="text-xl font-black text-slate-900">1 Master Flow</div>
              </div>
              <div className="bg-white p-4 rounded-2xl border border-slate-200 space-y-1 shadow-sm">
                <span className="text-[10px] uppercase font-mono font-bold text-slate-500">Workflow Nodes</span>
                <div className="text-xl font-black text-emerald-700">{nodes.length} Nodes</div>
              </div>
              <div className="bg-white p-4 rounded-2xl border border-slate-200 space-y-1 shadow-sm">
                <span className="text-[10px] uppercase font-mono font-bold text-slate-500">Node Connections</span>
                <div className="text-xl font-black text-teal-700">{edges.length} Edges</div>
              </div>
              <div className="bg-white p-4 rounded-2xl border border-slate-200 space-y-1 shadow-sm">
                <span className="text-[10px] uppercase font-mono font-bold text-slate-500">Message Templates</span>
                <div className="text-xl font-black text-green-700">{automationMessageTemplates.length} Templates</div>
              </div>
            </div>
            ...
            <div className="bg-white border border-slate-200 rounded-3xl p-6 space-y-4 shadow-sm">
              <div className="flex items-center justify-between">
                ...
                <button 
                  onClick={() => {
                    navigator.clipboard.writeText(AUTOMATION_SQL_SCHEMA);
