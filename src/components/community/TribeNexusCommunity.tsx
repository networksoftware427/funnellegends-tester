...
export const TribeNexusCommunity: React.FC<TribeNexusCommunityProps> = ({ defaultSpaceSlug = 'all' }) => {
  ...
  return (
    <div className="flex-1 bg-gray-50 text-gray-900 overflow-y-auto flex flex-col">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-2xl font-black text-slate-900 tracking-tight">TribeNexus</h1>
              <span className="text-[10px] uppercase font-mono font-bold bg-white/20 text-slate-900 border border-white/30 px-2 py-0.5 rounded">
      ...
      {isCreatingSpace && (
        <form onSubmit={handleCreateSpace} className="p-6 bg-white border border-emerald-500/50 rounded-3xl mb-6 space-y-4 shadow-2xl animate-fade-in">
          <div className="flex items-center justify-between border-b border-slate-200 pb-3">
            <h3 className="text-sm font-bold text-slate-900 flex items-center gap-2">
              <Plus className="w-4 h-4 text-emerald-400" />
              Create a New Community Space
            </h3>
            <span className="text-[10px] text-slate-600 font-mono">ORGANIZE YOUR TRIBE</span>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className="text-xs font-bold text-slate-700 block mb-1">Space Name</label>
              <input 
                type="text"
                placeholder="e.g. 📢 Announcements"
                value={newSpaceName}
                onChange={(e) => setNewSpaceName(e.target.value)}
                className="w-full bg-white border border-slate-200 rounded-xl px-3.5 py-2 text-xs text-slate-900 placeholder-slate-500 focus:outline-none focus:border-emerald-500"
              />
            </div>
            <div>
          ...
          <div className="flex justify-end gap-2 pt-2 border-t border-slate-200">
            <button 
              ...
              className="px-4 py-2 bg-white hover:bg-slate-50 text-slate-600 text-xs font-bold rounded-xl border border-slate-200"
            >
              Cancel
            </button>
            <button 
      ...
      {isCreatingPost && (
        <form onSubmit={handleCreatePost} className="p-6 bg-white border border-purple-500/50 rounded-3xl mb-6 space-y-4 shadow-2xl animate-fade-in">
          <div className="flex items-center justify-between border-b border-slate-200 pb-3">
            <h3 className="text-sm font-bold text-slate-900 flex items-center gap-2">
              <Sparkles className="w-4 h-4 text-purple-400" />
              Share Rich Post or Media with Tribe
            </h3>
            <span className="text-[10px] text-slate-600 font-mono">DISTRACTION-FREE POSTING</span>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <div className="sm:col-span-2">
              ...
              <input 
                ...
                className="w-full bg-white border border-slate-200 rounded-xl px-3.5 py-2 text-xs text-slate-900 placeholder-slate-500 focus:outline-none focus:border-purple-500"
              />
            </div>
            <div>
              <label className="text-xs font-bold text-slate-700 block mb-1">Select Space Topic</label>
              <select 
                value={newPostSpaceSlug}
                onChange={(e) => setNewPostSpaceSlug(e.target.value)}
                className="w-full bg-white border border-slate-200 rounded-xl px-3.5 py-2 text-xs text-slate-900 focus:outline-none focus:border-purple-500"
              >
          ...
          <div>
            <label className="text-xs font-bold text-slate-700 block mb-1">Post Content & Insights</label>
            <textarea 
              rows={4}
              placeholder="Write your detailed post breakdown, strategy, or question for the mastermind community..."
              value={newPostContent}
              onChange={(e) => setNewPostContent(e.target.value)}
              className="w-full bg-white border border-slate-200 rounded-xl p-3 text-xs text-slate-900 placeholder-slate-500 focus:outline-none focus:border-purple-500"
            />
          </div>

          <div>
            <label className="text-xs font-bold text-slate-700 block mb-1">Attach Media Image URL (Optional)</label>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-1 space-y-4">
          <div className="bg-white border border-slate-200 rounded-3xl p-5 space-y-4 shadow-xl">
            ...
            <div className="space-y-1.5">
              <button 
                ...
                className={`w-full p-2.5 rounded-xl text-left text-xs font-bold flex items-center justify-between transition-all ${selectedSpaceSlug === 'all' ? 'bg-purple-600 text-white shadow' : 'bg-white text-slate-600 hover:text-slate-900 border border-slate-850'}`}
              >
                <div className="flex items-center gap-2">
                  <span>🌐</span>
                  <span>All Spaces & Feeds</span>
                </div>
                <span className="text-[10px] font-mono bg-white px-2 py-0.5 rounded text-slate-700">{posts.length}</span>
              </button>
              ...
              {spaces.map((sp) => (
                <button 
                  ...
                  className={`w-full p-2.5 rounded-xl text-left text-xs font-bold flex items-center justify-between transition-all ${selectedSpaceSlug === sp.slug ? 'bg-purple-600 text-white shadow' : 'bg-white text-slate-600 hover:text-slate-900 border border-slate-850'}`}
                >
                  <div className="flex items-center gap-2 truncate">
                    <span>{sp.icon}</span>
                    <span className="truncate">#{sp.slug}</span>
                  </div>
                  <span className="text-[10px] font-mono bg-white px-2 py-0.5 rounded text-slate-700">{sp.badgeCount}</span>
                </button>
              ))}
            </div>
          </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          {filteredPosts.length === 0 ? (
            <div className="p-12 text-center bg-white border border-slate-200 rounded-3xl text-xs text-slate-500">
              No posts found in this space topic. Click "Create New Community Post" above to start the conversation!
            </div>
          ) : (
            filteredPosts.map((post) => (
              <div key={post.id} className="p-6 bg-white border border-slate-200 hover:border-purple-500/40 rounded-3xl space-y-4 shadow-xl transition-all">
                {/* Author Bar */}
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="font-extrabold text-xs text-slate-900">{post.authorName}</span>
                ...
                <div className="flex items-center justify-between border-t border-slate-200/80 pt-3 text-xs">
                  <div className="flex items-center gap-3">
                    <button 
                      ...
                      className="px-3 py-1.5 bg-white hover:bg-purple-950 text-slate-700 hover:text-purple-300 border border-slate-200 hover:border-purple-800 rounded-xl font-extrabold flex items-center gap-1.5 transition-all"
                    >
                  ...
                  <div className="flex gap-1">
                    {post.tags.map(t => (
                      <span key={t} className="text-[10px] font-mono text-slate-500 bg-white px-2 py-0.5 rounded">#{t}</span>
                    ))}
                  </div>
                </div>

                {/* Comments Section */}
                <div className="p-4 bg-white rounded-2xl border border-slate-200 space-y-3 pt-4">
                  <div className="space-y-2.5">
                    {post.comments.map(c => (
                      <div key={c.id} className="flex items-start gap-2.5 text-xs p-2.5 bg-white rounded-xl border border-slate-200/60">
                  ...
                  <div className="flex gap-2 pt-2">
                    <input 
                      ...
                      className="flex-1 bg-white border border-slate-200 rounded-xl px-3 py-1.5 text-xs text-slate-900 placeholder-slate-500 focus:outline-none focus:border-purple-500"
                    />
                    <button 
        ...
        <div className="lg:col-span-1 space-y-4">
          <div className="bg-white border border-slate-200 rounded-3xl p-5 space-y-4 shadow-xl">
            ...
            <div className="space-y-3">
              {members.map(m => (
                <div key={m.id} className="p-3 bg-white rounded-2xl border border-slate-200 space-y-1.5">
                  <div className="flex items-center gap-3">
                  ...
                  <p className="text-[11px] text-slate-600 leading-tight">{m.bio}</p>
